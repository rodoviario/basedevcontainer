...
RUN apt-get update -y && apt-get install -y --no-install-recommends \
  build-essential file make git ninja-build gettext cmake unzip curl ca-certificates bash \
...
LABEL \
  org.opencontainers.image.authors="https://github.com/rodoviario" \
  org.opencontainers.image.created=$CREATED \
  org.opencontainers.image.version=$VERSION \
  org.opencontainers.image.revision=$COMMIT \
  org.opencontainers.image.url="https://github.com/rodoviario/basedevcontainer" \
  org.opencontainers.image.documentation="https://github.com/rodoviario/basedevcontainer" \
  org.opencontainers.image.source="https://github.com/rodoviario/basedevcontainer" \
...
RUN ln -fs "/usr/share/zoneinfo/${TIMEZONE}" /etc/localtime \ 
  ...
  bash nano locales wget git tmux python3 make fzf \
  && dpkg -i -- *.deb \
  && rm -f -- *.deb \
